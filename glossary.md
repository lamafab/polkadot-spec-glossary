---
title: "Polkadot Runtime Environment Spec Glossary (WIP)"
date: January 16, 2020
geometry: margin=8mm,landscape
output: pdf_document
fontsize: 9pt
---
...
|$StoredValue$|function to retrieve the value stored under a specific key in the state storage|$\left.\begin{array}{l c}StoredValue & \mathcal K \to \mathcal V\\& k \to \left\{\begin{array}{l c}v&if(k,v)\text{ exists in state storage}\\\phi&otherwise\end{array}\right.\end{array}\right.\newline\newline$where $K \subset \mathbb B$ and $\mathcal Vw \subset \mathbb B$ are respectively the set of all keys and values stored in the state storage|
|$KeyEncode(k)\newline\newline$$k_{enc}$|function to encode keys for labeling brnaches of the Trie|$k_{enc}:=(k_{enc1},...,k_{enc2n}):=KeyEncode(k)\newline\newline$such that:$\newline\newline KeyEncode(k):\left\{\begin{array}{l l l}\mathbb B & \to & Nibbles^4\\k:=(b_1,...,b_n) := & \to & (b^1_1,b^2_1,b^1_2,b^2_2,...,b^1_n,b^2_n)\\ & \to &:=(k_{enc_1},...,k_{enc2n})\end{array}\right.\newline\newline$where $Nibble^4$ is the set of all nibbles of 4-bit arrays and $b^1_i$ and $b^2_i$ are 4-bit nibbles, which are the big endian representation of $b_i$:$\newline\newline (b^1_i,b^2_i):=(b_i/16,b_i,mod16)\newline\newline$where mod is the remainder and / is the integer division operators.|
|$pk_N$|TODO||
|$pk^{Agr}_N$|TODO||
...
|$ChildrenBitmap$||$ChildrenBitmap:\left.\begin{array}{l}\mathcal N_b \to \mathbb B_2\\N \to (b_{15},...,b_8,b_7,...b_0)_2\end{array}\right.\newline\newline$where$\newline\newline b_i:=\left\{\begin{array}{l c}1 & \exists N_c \in \mathcal N:k_{Nc} = k_{N_b} \parallel i \parallel pk_{Nc}\\0 & otherwise\end{array}\right.$|
|$H(N)$|the Merkle value of $N$|$\left.\begin{array}{l}H:\mathbb B \to \mathbb B_{32}\\H(N):\left\{\begin{array}{l l}v_N & \parallel v_N \parallel < 32\\Blake2b(v_N) & \parallel v_N \parallel \geq 32\end{array}\right.\end{array}\right.\newline\newline$Where $v_N$ is the node value of $N$ and $0_{32-\parallel v_N \parallel}$ an all zero byte array of length $32-\parallel v_N \parallel$. The Merkle hash of the Trie is defined as: $Blake2b(H(R))$ where $R$ is the root of the Trie.|
...
|$Enc^{Len}_{SC}$|SCALE length encoding aka. compact encoding of non-negative interger numbers of varying sized prominently in an encoding length of arrays|$Enc^{Len}_{SC}: \mathbb N \to \mathbb B \newline n \to b\left\{\begin{array}{l l}l_1 & 0 \leq n < 2^6\\i_1i_2 & 2^6 \leq n < 2^{14} \\j_1j_2j_3 & 2^{14} \leq n < 2^{30} \\k_1k_2...k_m & 2^{30} \leq n\end{array}\right.\newline\newline$in where the least significant bits of the first byte of byte array $b$ are defined as follows:$\newline\newline \left.\begin{array}{r r}l^1_1l^0_1 = 00\\i^1_1i^0_1 = 01\\j^1_1j^0_1 = 10\\k^1_1k^0_1 = 11\end{array}\right.\newline\newline$and the rest of the bits of $b$ store the value of $n$ in little-endian format in base-2 as follows:$\newline\newline \left.\begin{array}{l l}l^7_1...l^3_1l^2_1 & n < 2^6\\i^7_2...i^0_2i^7_1...i^2_1 & 2^6 \leq n < 2^{14}\\j^7_4...j^0_4j^7_3...j^7_1...j^2_1 & 2^{14} \leq n < 2^{30}\\k_2+k_32^8+k_42^{2\times8}+...+k_m2^{(m-2)\times8} & 2^{30} \leq n\end{array}\right\}:=n\newline\newline$such that:$\newline\newline k^7_1..k^3_1k^2_1:=m-4$|
